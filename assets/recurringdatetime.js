...
(function($) {
	$.fn.datetime = function() {
        // we now pass in the .field-datetime because it contains multiple
        // labels, each containing an individual span form input. thus, we want
        // access to all its contained labels
		return this.children().each(function() {
			var _label = $(this);
			var _field = _label.parent('.field-datetime');
			var _input = _field.find('input');

			// language
            // extract the json encoded settings frome the hidden input element of the field
			if(Calendar.settings == undefined) Calendar.settings = eval('(' + _input.filter(':hidden').val() + ')');
			// init
            // remove the add link is the calendar is set not set to multiple
